...
Phase 2.5 统一存储  ────────────────────────┘ （独立路径，但 Phase 3.1 强依赖）
                    │
Phase 3.1 自定义 Provider ────────────────── （强依赖：Phase 2.5）
Phase 3.2 健康检查  ──────────────────────── （独立）
...
|------|------|--------|---------|---------|
| 1.1 | 错误码标准化 | P0 | 1 天 | 无 |
| 1.2 | 传输层重试 | P0 | 1 天 | 1.1 |
| 2.1 | 模型元数据系统 | P1 | 1.5 天 | 无 |
| 2.2 | 动态模型发现 | P1 | 1.5 天 | 2.5（缓存部分） |
| 2.3 | API Key 安全增强 | P1 | 1 天 | 无 |
| 2.5 | 统一本地存储层 | P1 | 2 天 | 无 |
| 3.1 | 用户自定义 Provider | P2 | 1 天 | 2.5 |
| 3.2 | 健康检查 | P2 | 0.5 天 | 无 |
| 3.3 | 成本追踪 | P2（可选） | 1 天 | 2.1（可选） |

**总计**：Phase 1 约 **2 天**，Phase 2 约 **7 天**，Phase 3 约 **2 天**
